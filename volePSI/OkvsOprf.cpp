...
namespace volePSI
{
    Proto OkvsOprfSender::send(u64 n, PRNG& prng, Socket& chl, u64 numThreads, bool reducedRounds)
    {
    ...
    block OkvsOprfSender::eval(block v)
    {
        block o;
        eval({ &v,1 }, { &o,1 }, 1);
        return o;
    ...
    Proto OkvsOprfSender::genVole(PRNG& prng, Socket& chl, bool reduceRounds)
    {
        if (reduceRounds)
    ...
    Proto OkvsOprfReceiver::receive(span<const block> values, span<block> outputs, PRNG& prng, Socket& chl, u64 numThreads, bool reducedRounds)
    {
    ...
    Proto OkvsOprfReceiver::genVole(u64 n, PRNG& prng, Socket& chl, bool reducedRounds)
    {
        if (reducedRounds)
